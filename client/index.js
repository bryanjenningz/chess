...
const tileToUnicode = ({ piece, player }) => {
  switch (piece) {
    case PIECES.PAWN:
      return ({ [PLAYERS.WHITE]: '♙', [PLAYERS.BLACK]: '♟' })[player]
    case PIECES.ROOK:
      return ({ [PLAYERS.WHITE]: '♖', [PLAYERS.BLACK]: '♜' })[player]
    case PIECES.KNIGHT:
      return ({ [PLAYERS.WHITE]: '♘', [PLAYERS.BLACK]: '♞' })[player]
...
const selectedTileMoves = ({ x, y }, board) => {
  ...
  const isValidMove = ({ x, y }) =>
    x >= 0 &&
      ...
      y >= 0 &&
      y <= 7 &&
      (board[y][x] === null || board[y][x].player !== player)
    ...
    case PIECES.PAWN:
      if (player === PLAYERS.WHITE) {
        ...
        if (
          ...
          x - 1 >= 0 &&
          board[y - 1][x - 1] &&
          board[y - 1][x - 1].player === PLAYERS.BLACK
        ) {
        ...
        if (
          ...
          x - 1 >= 0 &&
          board[y + 1][x - 1] &&
          board[y + 1][x - 1].player === PLAYERS.WHITE
        ) {
          moves.push({ x: x - 1, y: y + 1 })
        }
        if (
    ...
    row.map(
      (tile, x) =>
        x === destination.x && y === destination.y
          ? board[tileSelected.y][tileSelected.x]
          : x === tileSelected.x && y === tileSelected.y ? null : tile
    )),
  gameover: (
    board[destination.y][destination.x] &&
      ...
      <div>
        ...
        {board.map((row, y) => (
          <div key={y} style={{ display: 'flex' }}>
            {row.map((tile, x) => {
              const backgroundColor = x === tileSelected.x &&
                y === tileSelected.y
                ? 'skyblue'
                : tileMoves.some(
                    tileMove => tileMove.x === x && tileMove.y === y
                  )
                    ? 'dodgerblue'
                    : (x + y) % 2 === 0 ? '#eee' : '#444'
              return (
                <div
                  ...
                  onClick={() => {
                    if (!gameover) {
                      const tileThatIsCurrentlySelected = board[
                        tileSelected.y
                      ] &&
                        board[tileSelected.y][tileSelected.x]
                      const isPossibleMove = tileMoves.some(
                        move => move.x === x && move.y === y
                      )
                      if (
                        tileThatIsCurrentlySelected &&
                        tileThatIsCurrentlySelected.player === playerTurn &&
                        isPossibleMove
                      ) {
